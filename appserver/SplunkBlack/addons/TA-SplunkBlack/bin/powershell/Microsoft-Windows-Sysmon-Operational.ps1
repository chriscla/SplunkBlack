...
#$positionFilePath = Join-Path -Path $env:SPLUNK_HOME "etc\apps\TA-SplunkBlack\bin\powershell"
#$positionFile = Join-Path $positionFilePath $positionFileName
$positionFile = $positionFileName
...
try
{
    ...
    $events = Get-WinEvent -LogName $logName -FilterXPath "*[System[(EventRecordID > $lastRecordID) and (EventID=1)]]" -Oldest -ea SilentlyContinue | Sort-Object RecordId
